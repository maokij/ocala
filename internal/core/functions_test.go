	...
	t.Run("error", func(t *testing.T) {
		es := []string{
			"unknown namespace nothing", `flat!
				import nothing
			`,
			"loop counter is must be constexpr", `flat!
				expand-loop A { NOP }
			`,
			"loop counter is must be integer", `flat!
				expand-loop "A" { NOP }
			`,
...
func TestCompileSpecials(t *testing.T) {
	...
	t.Run("ok: __FILE__", func(t *testing.T) {
		g := ttarch.BuildGenerator(ttarch.BuildCompiler(), `flat!
			db $$(__FILE__)
		`)
	...
	t.Run("error", func(t *testing.T) {
		es := []string{
			...
			"invalid else-if form", `flat!
				if (1 == 1) { db 0 } else if
			`,
			"cond is must be constexpr", `flat!
	...
	t.Run("error", func(t *testing.T) {
		es := []string{
			"case: 3+ argument(s) required, but given 1", `flat!
				case 1
			`,
			"value is must be constexpr", `flat!
				case A when 1 {}
			`,
			"invalid case-when form", `flat!
				case 1 when 1 {} when
			`,
			"pattern is must be constexpr", `flat!
				case 1 when EQ? {}
			`,
...
func TestCompilePragma(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		es := []string{
			...
			`,
			"value is must be integer", `flat!
				module Mod { pragma list-constants "1" }
			`,
			"message is must be constexpr", `flat!
				module Mod { pragma list-constants 1 A }
			`,
			"message is must be string", `flat!
				module Mod { pragma list-constants 1 1 }
			`,
			"comment is must be constexpr", `flat!
				pragma comment A
			`,
			"comment is must be string", `flat!
				pragma comment 1
			`,
		}
		for x := 0; x < len(es); x += 2 {
...
func TestCompileMakeCounter(t *testing.T) {
	...
	t.Run("error", func(t *testing.T) {
		es := []string{
			"start is must be constexpr", `flat!
				make-counter c A
			`,
			"start is must be integer", `flat!
				make-counter c "A"
			`,
		}
		for x := 0; x < len(es); x += 2 {
